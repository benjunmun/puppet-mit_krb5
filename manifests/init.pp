...
class mit_krb5(
  ...
  Optional[Array[String]]   $default_tgs_enctypes     = undef,
  Optional[Array[String]]   $default_tkt_enctypes     = undef,
  Optional[String]          $default_ccache_name      = undef,
  Optional[Array[String]]   $permitted_enctypes       = undef,
  Optional[Boolean]         $allow_weak_crypto        = undef,
  Optional[String]          $clockskew                = undef,
  Optional[Boolean]         $ignore_acceptor_hostname = undef,
  Optional[Boolean]         $k5login_authoritative    = undef,
  Optional[String]          $k5login_directory        = undef,
  Optional[Integer[0,1]]    $kdc_timesync             = undef,
  Optional[Integer]         $kdc_req_checksum_type    = undef,
  Optional[Integer]         $ap_req_checksum_type     = undef,
  Optional[Integer]         $safe_checksum_type       = undef,
  Optional[String]          $preferred_preauth_types  = undef,
  Optional[Integer]         $ccache_type              = undef,
  Optional[Boolean]         $dns_lookup_kdc           = undef,
  Optional[Boolean]         $dns_lookup_realm         = undef,
  Optional[Boolean]         $dns_fallback             = undef,
  Optional[Integer]         $realm_try_domains        = undef,
  Optional[Array[String]]   $extra_addresses          = undef,
  Optional[Integer]         $udp_preference_limit     = undef,
